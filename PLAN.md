...
- [ ] **Move onto the upstream Flight Lab theme.** Steve's instruction, relayed 20 Sep by the control course session; confirm the timing with him. Do it *after* Friday: it buys nothing visually and costs risk in a repo about to go live. Two parts: (a) slides — `"flightlab-marp-template": "github:BristolFlightLab/flightlab-marp-template#v2.0.0"`, plus `themeSet: node_modules/flightlab-marp-template/themes` in `.marprc.yml` and `npm uninstall marp-template`, since the package is renamed and the installed path moves; (b) site — add the theme as a submodule and delete this repo's `docs/stylesheets/flightlab.css` and the five shared SVGs in `docs/assets/brand/`, keeping AVDASI2's own logo and favicon
- [ ] **Flight Lab theme repos, when convenient** (noted 20 Sep). The shared branding now lives in `BristolFlightLab/flightlab-brand`, with `flightlab-zensical-theme` (v2.0.0) and `flightlab-marp-template` (v1.1.0, renamed from `marp-template`). Nothing here breaks: GitHub redirects the old URL and the npm package name is unchanged. When bumping, pin `github:BristolFlightLab/flightlab-marp-template#v1.1.0` (no visual change), and consider dropping this repo's copies of `docs/stylesheets/flightlab.css` and `docs/assets/brand/` in favour of the theme as a submodule, as CADE30008 has done. Its session has the gotchas: use theme **v2.0.0**, not v1.0.0, which published the theme's own README and LICENCE onto the site; point `custom_dir` at `theme/dist`; keep `stylesheets/` and `assets/` siblings; rewrite the path to `../theme/dist` for the `.live` config; set `submodules: true` in CI checkout; and test a clone *without* submodules too, confirming it fails loudly rather than quietly deploying an unbranded site
